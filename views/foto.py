  ...
  if text_input:
      ...
      if st.button("Carica su GSheet"):
          ...
          if skus_to_append_clean:
              ...
              rows_to_append = [[f"'{sku}", f"{oggi}", f"=IMAGE(SOSTITUISCI(SETTINGS!$B$4;"*SKU*";MINUSC($A{(sheet_len+1)+1})))", f"=SE(VAL.NON.DISP(CONFRONTA(INDIRETTO(\"LISTA!$D\"&CONFRONTA($A{(sheet_len+1)+1};LISTA!A:A;0));SPLIT(SETTINGS(\"brandMatias\");\",\");0));SE(VAL.NON.DISP(CONFRONTA(INDIRETTO(\"LISTA!$D\"&CONFRONTA($A{(sheet_len+1)+1};LISTA!A:A;0));SPLIT(SETTINGS(\"brandMatteo\");\",\");0));\"\";\"MATTEO\");\"MATIAS\")", "ECOM"] for i, sku in enumerate(skus_to_append_clean)]
