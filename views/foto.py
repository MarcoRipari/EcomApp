  ...
  if text_input:
      ...
      if st.button("Carica su GSheet"):
          ...
          if skus_to_append_clean:
              ...
              rows_to_append = [[f"'{sku}", f"{oggi}", f"=IMAGE(SOSTITUISCI(SETTINGS!$B$4;\"*SKU*\";MINUSC($A{(sheet_len+1)+i})))", f"=SE(VAL.NON.DISP(CONFRONTA($D{(sheet_len+1)+i};SPLIT(SETTINGS(\"brandMatias\");\",\");0));SE(VAL.NON.DISP(CONFRONTA($D{sheet_len+1};SPLIT(SETTINGS(\"brandMatteo\");\",\");0));\"\";\"MATTEO\");\"MATIAS\")", "ECOM"] for i, sku in enumerate(skus_to_append_clean)]
