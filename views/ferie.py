...
def dashboard_dipendente():
    ...
    st.write(mio_storico_completo.columns.values.tolist())
    # mio_storico = mio_storico_completo[(mio_storico_completo["_fine"] <= oggi) & (mio_storico_completo["DETTAGLIO"].fillna("").str.lower() != "rettifica")].copy() if not mio_storico_completo.empty else mio_storico_completo
    mio_storico = (
        mio_storico_completo[
            (mio_storico_completo["_fine"] <= oggi) & 
            (mio_storico_completo["DETTAGLIO"].astype(str).str.lower() != "rettifica")
        ].copy() 
        if not mio_storico_completo.empty 
        else mio_storico_completo
    )
